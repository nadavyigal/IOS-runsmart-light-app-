...
struct ResumePreviewView: View {
    ...
    @State private var showDesigned = false
    ...
    private var viewToggle: some View {
        HStack(spacing: 0) {
            toggleTab(label: "Optimized", isSelected: !showDesigned) {
                withAnimation(.easeInOut(duration: 0.2)) { showDesigned = false }
            }
            toggleTab(label: "Designed", isSelected: showDesigned) {
                withAnimation(.easeInOut(duration: 0.2)) { showDesigned = true }
            }
        }
        .padding(3)
    ...
    private var resumeSheet: some View {
        ...
        .gesture(
            MagnificationGesture()
                .onChanged { val in scale = max(0.8, min(1.6, val)) }
                .onEnded { _ in withAnimation(.spring(duration: 0.3)) { scale = 1.0 } }
        )
    }

    private var designedContent: some View {
    ...
    private var fileRow: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(showDesigned ? "Designed_Resume.pdf" : "Optimized_Resume.pdf")
                    .font(.subheadline.weight(.semibold))
    ...
    private var exportButtons: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                exportButton(icon: "arrow.down.circle.fill", label: "Download PDF", isLoading: isDownloading) {
    ...
    private func saveVersion() async {
        ...
        _ = try? await apiClient.postJSON(
            ...
            body: ["optimizationId": optimizationId],
            token: token
    ...
    private func fetchPDF(optimizationId: String, token: String) async throws -> URL {
        let url = BackendConfig.apiBaseURL
            .appendingPathComponent("api/download/\(optimizationId)")
