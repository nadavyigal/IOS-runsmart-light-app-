...
## P0 UPDATE (added 2026-07-19 evening): the door may be broken, not just unpersuasive

A second pass re-examined the two persons this plan originally discarded as a "sign-in-debug pair". That exclusion threw away the most diagnostic evidence in the dataset. Corrected reading:

**Every fresh install that actually attempted Sign in with Apple failed. Seven non-cancel `ASAuthorizationError` code-1000 failures across three distinct devices and two builds, with zero successes on any of them:**

| Person | Device | Build | Failures | Result |
|---|---|---|---|---|
| `03766171` | iPhone99,7 (not shipped Apple hardware) | 1.0.9 public | 2 | no success, silence |
| `26140ca4` | iPhone12,1 (iPhone XR, real hardware) | 1.0.9 public | 3 | no success, silence |
| `8ea67ea8` | iPhone17,2 (real hardware) | **1.1.0 build 24** (2026-07-19 11:03 UTC) | 2 | no success, silence |

The only `sign_in_completed` on 1.0.9 (2026-07-16) belongs to `iPhone14,5` — the founder's already-provisioned iPhone 13. So the production path *can* work, but no observed fresh install has completed it.

**Supabase evidence (read-only, 2026-07-19):** the `auth` logs covering today's 11:03-11:04 UTC failures contain **no `/token` request at all** — only the founder's web refresh-token traffic from `runsmart-ai.com`. The failure therefore happens **on-device, between the app and Apple, before Supabase is ever contacted.** This rules out `provider_disabled` (the failure class logged in `~/.claude/LEARNINGS.md` 2026-06-12) for that window. Older 07-15 failures fall outside Supabase's 24h log retention and cannot be checked retroactively.

**What this is and is not.** Code 1000 is `ASAuthorizationError.unknown`, which is also produced by a device with no iCloud account signed in and by locally-installed builds whose provisioning lacks the Sign in with Apple capability. Build 24 was **not** an App Store install, so that data point is weak on its own. Verdict: **a strong, unresolved P0 signal — not a proven production outage.** The 2026-07-19 device test that was supposed to settle it is **BLOCKED** (`docs/qa/reports/release-1.0.9-build23-siwa-device-test-2026-07-19.md`): the founder's device went `unavailable` in `devicectl` before SignInView was ever reached, so no clean attempt was recorded.

**Consequence for sequencing:** if sign-in is broken for new users, S2 (showing value before the account ask) treats a symptom while the door stays locked. Settle this first — it is one clean device attempt (~20 minutes), and it reorders everything below.
...
1. Users bounce because they are asked to create an account before seeing any product value. Note the P0 update above: for the three fresh installs that *did* tap the button, sign-in failed 100% of the time, so "unwilling" and "unable" are both live hypotheses and the wall may be functionally broken rather than merely unpersuasive.
...
Re-sequenced 2026-07-19 evening: **S0 now precedes everything** because it can invalidate the premise of S2.

0. **S0. Settle whether Sign in with Apple actually works for new users (XS, ~20 minutes, founder-only).** Not an engineering story. On a device signed into iCloud: delete the app, install **public 1.0.9 (23) from the App Store** (not a local build), tap Sign in with Apple exactly once, and record the UTC time plus the exact on-screen text. Then check PostHog for a matching `sign_in_completed` or `sign_in_failed` within that minute. **If it fails on a clean iCloud-signed device, this is a production outage and outranks every other item in the portfolio** — next step would be inspecting the App ID's Sign in with Apple capability and the ASC provisioning for the shipped build, since Supabase has already been ruled out for the 07-19 window. **If it succeeds**, the wall is a persuasion problem, S2 keeps its priority, and the seven failures get attributed to environment (no iCloud account / non-App-Store builds). Either answer is worth far more than the twenty minutes. The prior attempt is documented as blocked in `docs/qa/reports/release-1.0.9-build23-siwa-device-test-2026-07-19.md`.

Then, in this order:

1. **S1. Instrument the black hole (S, 1 day; target: next build, 1.1.1).** Attach `error_domain`/`error_code` to every wall event so S0's question answers itself for every future user without a device repro. Add `sign_in_wall_viewed`, `sign_in_tapped`, and `sign_in_wall_abandoned` with defined semantics: fires at most once per session, only when the app backgrounds (or terminates) after >=5 seconds on the wall with no sign-in attempt; returning to foreground and then attempting sign-in does not retro-emit. Add explicit screen names. Fix the `plan_generation_failed`/`succeeded` double-fire. Session replay is desirable but **gated on privacy guardrails first**: documented consent/opt-out, masking of text input and any health-adjacent surfaces, retention limit, and access control; if that takes longer than a day, ship the events without replay rather than delaying. Metric moved: none directly; it converts every later fix from faith to measurement.
