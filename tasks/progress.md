...
Status: WP-37 (RunSmart Record-Run UX Polish) **CLOSED 2026-07-09** — all 8 stories (S1-S8) confirmed merged to `main` at `4476a7d` via PR #73-#80; local `main` fast-forwarded to match `origin/main` exactly. PR #53 (stale Build-14 stranded-branch cleanup) **closed without merging 2026-07-09** — all three of its claimed fixes (xcconfig `SUPABASE_URL` escape, `coach_message` length guard, HealthKit bulk-upsert retry fallback) independently re-verified already present verbatim on `main`. WP-15 fix shipped to App Store Connect as **1.0.7 (21)** on 2026-07-05. Post-fix activation cohort readout is **pending** (needs ~3-7 days live on build 21). WP-34 Garmin credential-guard: closed, explicitly parked per EXD-019 (2026-07-05) — commit unrecoverable, not being reimplemented; revisit 2026-08-01 with EXD-015 reread. WP-27 Garmin Gate-4 evidence cleanup complete on main; founder-run screenshots still pending.
...
Last Completed Story: 2026-07-09 — WP-37 closeout verification: local worktree synced to `origin/main` (`4476a7d`); removed 35 stray Finder `* 2.*` duplicate files under `IOS RunSmart app/`, `tasks/`, `docs/`, `supabase/` (verified byte-identical to originals via `diff` before deletion — these were the compile-blocking duplicates flagged in WP-37's own build note); clean Debug build **SUCCEEDED** on iPhone 17 sim with zero source duplicates present; app installed and launched successfully (`-RUNSMART_DEMO_MODE`) on both iPhone 17 and iPhone SE sims with no crash. Confirmed S3b (iPhone-SE control-row label clipping, folded into PR #77) fix is present in code: `LiveRunView.swift` wraps its body in `ScrollView` with `minHeight: proxy.size.height`.
Next Recommended Story: Re-run PostHog funnel on **2026-07-08+** for build-21-only users (`filterTestAccounts=true`). Then WP-27 Gate-4 screenshots if Garmin path resumes. WP-37 follow-ups not yet scoped as stories: S6/S7 unfixed-state polish items noted in the WP-37 tracker's "Explicitly deferred" section (live per-km splits, elapsed-vs-moving time label) remain open if the founder wants to prioritize them.
...
Blockers: (1) Post-fix cohort not yet measurable same-day as upload. (2) **RESOLVED 2026-07-09** — local main worktree's Finder `* 2.swift` duplicate files (previously blocked archive per 2026-07-05 note) have been removed; a fresh Debug build now succeeds directly from this worktree without needing a clean detached checkout. Re-verify before the next Release archive in case Finder recreates the duplicates.
Last Validation: 2026-07-09 — Debug build **SUCCEEDED** (`xcodebuild ... -configuration Debug build`, iPhone 17 sim, 91% cache hit) from the synced, duplicate-free local worktree. App installed + launched clean on iPhone 17 and iPhone SE sims. Prior Release validation: 2026-07-05 — Release archive **SUCCEEDED** (clean worktree, ~10 min). ASC upload **SUCCEEDED** (`ExportOptionsAppStoreUpload.plist`). Archive metadata: `RunSmart` / `com.runsmart.lite` / `1.0.7` / `21` / `ITSAppUsesNonExemptEncryption=false` / dSYM present. Known HKWorkout deprecation warning only.
PM Artifacts: Activation funnel events in `.agent-os/distribution/analytics-instrumentation-spec.md`; WP-34 incident in `tasks/ERRORS.md`. WP-37 tracker: Agentic OS repo `executive-os/work-packets/WP-37-runsmart-record-run-ux.md` (untracked in that repo by design, per WP-37 session note).
