**WP-37 S3 — Paused control row must fit the screen (P1), 2026-07-08:** Fixed `LiveRunView`'s paused control row overflowing every supported width. Root cause confirmed exactly as scoped: Resume(112) + Finish(78) + Coach(78) + Discard(78) + 3×18 spacing + 36 padding = 436pt, wider than iPhone 17's 402pt (worse on smaller phones). Fix (smallest diff, exactly per WP-37's proposed change): the row never shows 4 buttons — while `.paused`, Discard replaces Coach instead of appending a fourth button (Coach/mute is inert while paused since `VoiceCoachService` is itself paused, so there's nothing to mute). New max width: 112+78+78+2×18+36 = 340pt on every phase. **Validation:** clean worktree `/tmp/rs-wp37-s3` (branched off post-S1/S2 `main`), Debug build on iPhone 17 sim **SUCCEEDED**. No existing unit/UI tests target `LiveRunView` (pure layout, no store); the diff only swaps which button renders in the paused branch — `onPauseResume`/`onFinish`/`onDiscard`/voice-coach action closures are untouched. **Device QA deferred to the end-of-packet smoke pass per founder instruction** (2026-07-08): iPhone SE-width + iPhone 17 pause → all 3 buttons fully visible, no pill/banner shift, no truncation. Branch `claude/wp37-runsmart-s3-paused-controls`. Files: `Features/Run/LiveRunView.swift`. Next: S4 (visible cancel on finish/discard dialogs, needs device confirm) or S5 (real KM splits) — S4 explicitly requires physical-device verification before any code change per WP-37 constraints, so pairs naturally with the batched device QA pass.
