...
## Active Rules
- **An empty query result and "no users" are the same output, so never let a property name be a possible cause of it.** RunSmart registered `app_build` while Resumely registered `build_number` and the pinned contract named `build_number`. A contract query written once and pointed at both projects returned a correct answer for Resumely and an empty one for RunSmart — and at RunSmart's traffic, empty is also the *expected* answer. Two indistinguishable causes for one output is how a measurement stays broken for months. Fixed 2026-09-03 (WP-74 S2) by renaming to `build_number` rather than teaching the contract two names; the boundary date is recorded in `Analytics.buildIdentityProperties` and in the Builder OS contract note. When two systems must answer the same query, make the names identical, not the query tolerant.
- **Renaming a PostHog super property is two changes, not one: register the new key *and* `unregister` the old one.** Super properties are persisted on the device, and `register()` does not remove a key it no longer sets. Without the `unregister`, every install that ever ran the old code keeps emitting the old key frozen at whatever build it last saw — a value that never updates again. The stale form is strictly worse than the missing form, because it still splits and therefore still looks like data.
- **PostHog's own `$app_version`/`$app_build` are more reliable than a registered copy, and disagree with it at exactly the interesting moment.** Measured 2026-09-03 over 862 events: `$app_version` was present on 862, the registered `app_version` on 831. On `Application Updated`, 9 of 10 events carried the registered pair one release *behind* the `$`-prefixed pair on the same event, because the update fires inside `setup(config)` before the new build re-registers. Prefer `$app_version`/`$app_build` for install and update attribution; the registered pair remains necessary for anything the RunSmart wrapper does not send. This extends the 2026-07-20 lesson about `Application Installed` from "carries nothing" to "`Application Updated` carries the wrong thing".
- **The cheapest measurement check is `max(timestamp)` versus the store release timestamp.** On 2026-09-03 the 1.1.7 (32) cohort had 142 events and 2 persons and looked like a real read; its last event was seven hours *before* the release, so all of it was pre-release. Run the pre-release check before quoting any cohort. It is step 3 of the pinned contract and it is now step 3 of `scripts/measurement_contract.py`.
- **A SwiftUI `onAppear` does not prove a screen is visible when an app-level launch overlay is above it.** RunSmart mounts `SignInView` under `RunSmartLaunchView`, roughly 0.32 seconds before the overlay finishes fading. A “wall reached” event emitted from the wall's `onAppear` therefore arrives before `activation_first_frame_rendered` and overstates the user-visible boundary. Emit reachability from the shell's post-overlay resolved-screen boundary; keep view-local events for component lifecycle only.
- **Tests that assert localized date labels must set the locale they expect.** `testPlanWeeksGroupByCalendarWeekAndTotalDistance` expected English while the shared simulator/runtime language produced Hebrew, and it failed identically on the activation branch and untouched `origin/main`. A fixed English literal without a pinned locale is environment-dependent, not a stable regression gate.
- **An XcodeBuildMCP timeout can leave its `xcodebuild` process running after the tool call returns.** Check the process and DerivedData before retrying; never start a second build against the same DerivedData while the first is alive. For a cold Release simulator check on Apple silicon, set `ONLY_ACTIVE_ARCH=YES`/arm64 so Xcode does not spend release time compiling an unnecessary x86_64 slice. On 2026-07-26, the first 1.1.4 Release call timed out while its child kept compiling; the narrowed arm64 rerun completed successfully.
- **Read the built app's configuration key names before scripting a live service smoke.** The current RunSmart bundle uses `SUPABASE_PUBLISHABLE_KEY`; assuming legacy `SUPABASE_ANON_KEY` or `SUPABASE_KEY` produced a false failed request before the key list was inspected. Resolve the exact key name from the built `Info.plist`, then read only that value into a non-printed process variable.
- **A zero-duration XCTest abort with `malloc: pointer being freed was not allocated` is a test-host crash, not an assertion result.** Export the xcresult diagnostics, confirm the process died before assertions, and rerun the exact cases on a clean alternate simulator runtime before attributing it to app code. On 2026-07-26, four unrelated auth tests aborted this way on iOS 26.3 and all four passed on iOS 26.5 without a source change.
- **A bare `ASAuthorizationError` code 1000 with `has_underlying_error: false` is not diagnostic by itself.** Reproduced on a clean simulator 2026-07-26: with no Apple Account signed in, tapping Sign in with Apple makes iOS show its own account alert and dismissing it returns a bare code 1000, matching the shape seen in the failing production sessions. A restricted or managed account can produce the same shape, but Apple's `.unknown` case can also cover other failures, so do not infer one root cause from the code alone. First verify nonce, scopes, controller, entitlement, and service configuration; if those links are sound, stop expecting more instrumentation to recover an underlying cause Apple did not provide. Independently of the exact cause, a hard-gated app needs a non-Apple sign-in path so one opaque provider failure cannot lock the user out.
- **Never ship a single authentication path in front of a hard gate.** `RunSmartLiteAppShell` gated the whole app on Sign in with Apple, so a precondition the app cannot see or control locked users out of software they had already installed. Any sole auth method is a single point of failure; the wall needs a second door before it needs better copy.
- **Put auth flow logic in a testable seam, not in the SwiftUI view.** The old sign-in surface had no seam at all — the only testable thing was a static copy-mapping function — which is part of why a total lockout was invisible from inside the codebase. `EmailSignInModel` takes an injectable gateway so every branch (success, confirmation-required, failure, re-submit) is reachable without a network.
- **Verifying auth end-to-end in the simulator: two traps.** (1) macOS press-and-hold turns synthetic typing into the accent picker, so only the first character lands — use `xcrun simctl pbcopy <udid>` then Cmd+V instead of typing. (2) A `SecureField` with `.newPassword` shows an "Automatic Strong Password cover view text" overlay that swallows input; SwiftUI reuses the field when you switch modes, so dismiss and reopen the sheet to get a clean `.password` field.
- **Supabase auth rows written by hand need empty strings, not NULL.** Inserting a QA user directly into `auth.users` fails sign-in with `500: Database error querying schema` / `Scan error on column "confirmation_token": converting NULL to string is unsupported`. GoTrue scans `confirmation_token`, `recovery_token`, `email_change_token_new`, `email_change_token_current`, `email_change`, `phone_change`, `phone_change_token`, `reauthentication_token` into non-optional strings — set them all to `''`.
- **App Store Review traffic can look like real users and can fake an activation signal if you let it.** PostHog 171597 carries a sequence of short sessions with ascending but sometimes repeated build numbers, rotating device models, `$is_testflight = False`, and no completed sign-ins. On 2026-07-26, the combined release timing, build/session pattern, TestFlight status, and known submission context showed that all three "real non-founder devices failing sign-in" cited by a work packet were review traffic. A pre-release non-TestFlight build proves only that the session was not public App Store traffic; use the other context before attributing it to review. Cupertino geography is corroborating evidence, never a required discriminator: a genuine user can be in Cupertino and a reviewer can be elsewhere. Note this is the mirror image of the 2026-07-19 global lesson: over-aggressive exclusion *deletes* signal, and no exclusion *manufactures* it. Partition traffic into review / TestFlight / other and confirm the buckets reconcile to the daily person totals.
- **Archive verification and public App Store telemetry verification are separate checks.** The uploaded `.xcarchive` under `~/Library/Developer/Xcode/Archives/<date>/` is the upload input; Apple re-signs and thins the store-delivered IPA. Inspecting the archive and comparing its `POSTHOG_API_KEY` byte-for-byte against the intended project token settles the empty-key failure mode without printing the secret, but it does not prove the public artifact emitted. Telemetry evidence must be tied to the expected RunSmart bundle identifier, build number, and PostHog project; a matching review or public session then proves that build's pipeline, while a launch of the public App Store install closes the remaining archive-to-store gap. Before calling post-release silence an outage, compare it with the product's normal traffic rate and prior zero-activity days.
- Permission prompt analytics must share the prompt's concurrency boundary. Checking `.notDetermined` independently in two concurrent callers can emit two requests and contradictory denied/granted terminals for one system prompt; serialize one in-flight authorization task and emit exactly one outcome.
- A direct-call unit test does not prove lifecycle-event deduplication in the integrated SwiftUI/auth flow. Re-verify once-per-lifecycle guards through the actual re-mount and completion transition, then confirm the production session has one start and one terminal.
- Treat connected PostHog project settings as sensitive output: project reads can include configuration tokens, so never paste raw responses into task memory or reports. In a shared multi-product project, filter RunSmart reads by the native app namespace and selected fields; event-name or version-only matches can silently attribute another product's traffic to RunSmart. **The MCP server can also default to the wrong project entirely** — it opened on ResumeBuilder (270848) on 2026-07-22 and returned that product's taxonomy for a RunSmart query; confirm the active project is Running coach (171597) before trusting any read.
- Before archiving, verify the built `.app`'s `Info.plist` carries the intended `CFBundleVersion` **and** a non-empty `POSTHOG_API_KEY`. `RunSmartSecrets.xcconfig` is gitignored, so a checkout without it builds and archives successfully while emitting zero analytics — indistinguishable from broken instrumentation once the build is public.
- Do not conclude a string is missing from a compiled binary because `strings` cannot find it. Swift stores literals of **15 UTF-8 bytes or fewer inline** (small-string optimization), so short event names like `sign_in_failed` (14) and `app_launched` (12) never reach the string table while longer ones do. Verify short identifiers with a runtime assertion instead.

- **Passing a secrets xcconfig with `xcodebuild -xcconfig` prints its resolved values in the build preamble.** Observed during the 1.1.2 archive: Xcode echoed the configured analytics key before compilation. Future rule: do not pass a secrets-only file through the command-line `-xcconfig` flag when logs are captured; place the gitignored file at the project's optional include path and run the archive with filtered output, verifying only that the archived plist value is non-empty.

- **A fresh worktree has no `RunSmartSecrets.xcconfig`, and the resulting build emits zero analytics without failing.** `RunSmartConfig.xcconfig` commits `POSTHOG_API_KEY` as empty and pulls the real value via `#include? "RunSmartSecrets.xcconfig"`, which is gitignored and therefore absent from any new worktree. The build succeeds, but `setupAnalyticsIfNeeded` bails at the `!token.isEmpty` guard and PostHog is never initialized — a silent no-op that looks exactly like a broken instrumentation change. Observed 2026-07-20 while device-verifying WP-51; caught before launch by checking the built `Info.plist` for a resolved key rather than assuming. **Before any analytics device test from a worktree:** copy `RunSmartSecrets.xcconfig` from the primary checkout, then verify the built `.app/Info.plist` has a non-empty `POSTHOG_API_KEY`.
- **PostHog `Application Installed` / `Application Updated` can never carry super properties.** They fire inside `PostHogSDK.shared.setup(config)`, which returns before any following `register()` call, so build identity cannot attach. Confirmed empirically 2026-07-20 (7/7 and 5/5 untagged). `register()` before `setup()` is a documented no-op, so the only fixes are capturing those events manually afterwards or splitting installs by first-seen `app_launched`. Relevant whenever install counts need splitting by build.
- **A silent `git checkout` failure in a worktree produces a convincing false green.** `git checkout <branch>` fails if that branch is already checked out in another worktree (`fatal: '<branch>' is already used by worktree at ...`). Inside an `&&`/`||` chain the failure is swallowed, the build proceeds against whatever HEAD actually is, and the suite passes — against the wrong tree. Observed 2026-07-20 validating PR #105: the run reported a clean 313/313 while testing `main`, not the PR. Before trusting any validation run: assert `git log --oneline -1` is the intended commit, grep the working tree for a symbol the change introduces, and confirm the **new tests exist by name** in the xcresult bundle. Never infer correctness from a pass count alone — 313 and 317 both look green. To test a branch already checked out elsewhere, use `git checkout --detach origin/<branch>`.
- **Do not merge an old PR that touches `tasks/progress.md`.** Task-memory files are prepend-heavy and rewritten every session, so any PR older than a few days carries a frozen snapshot that will revert live state on merge. Observed 2026-07-20: #96 and #87 both carried 1.0.9-era `progress.md`, and it was #87's *only* merge conflict. Salvage the real content (docs, code) onto a fresh branch off `main` and drop the task-memory hunks, then close the original with a pointer to the replacement.
- Never let a UI-state observer double as the analytics emitter for a lifecycle funnel. `PlanGenerationStore` emitted `plan_generation_succeeded`/`failed` from every terminal state transition, so two notifications arriving back to back produced a contradictory pair 19ms apart (six of them on device `0efa0d1b`) and one of every pair was false. Rule: a terminal funnel event must be matched to an observed start and must consume it, so unmatched or duplicate terminal signals emit nothing and the numerator can never exceed the denominator.
- Do not trust SwiftUI `$screen` autocapture to identify a screen: it emits the same generic `UIHostingController<...>` string for every screen in this app. Any event whose analysis depends on which screen it came from must carry an explicit `screen` property.
- Before accepting a test that pins a bug fix, re-run it against the pre-fix implementation and confirm it fails. A test written after the fix can pass for reasons unrelated to the defect.
...
- **Schema SQL in a code comment is not a migration.** `RouteRepository.swift` shipped with its CREATE TABLE statements as a comment ("run in the dashboard"); the tables were never created, every remote fetch failed, and the client's graceful local fallback made cloud persistence a silent no-op for the feature's entire life — users lost saved routes/benchmarks on reinstall with zero errors surfaced. When a client reads a table, verify the table exists in the live schema (`information_schema.tables`) and keep the DDL as a tracked migration file, not prose.
- **A SwiftUI `Button` nested inside another `Button` silently does not work.** The route card's "Details" control was declared inside the card's selection `Button`; the tap resolved to a button in the accessibility tree but fired nothing, and the outer `.accessibilityElement(children: .combine)` additionally hid it from VoiceOver. Device-confirmed both before (dead tap) and after (opens route detail). Render secondary actions as **siblings** in a `ZStack` overlay, never nested — and treat "the tap registers but nothing happens" as a nesting smell before hunting the action's body.
- **RLS `WITH CHECK (user_id = auth.uid())` does not constrain what a row *points at*.** A child table whose FK only requires the referenced row to exist lets an authenticated user reference another user's row while passing RLS with their own `user_id`. Enforce ownership with a composite FK (`(child_fk, user_id) REFERENCES parent(id, user_id)`), which needs a `UNIQUE (id, user_id)` on the parent. Verify it by *attempting* the cross-user insert and asserting rejection; a structural constraint listing does not prove enforcement.
- **After a red-check that rebuilds from a stashed tree, DerivedData Products holds the pre-fix binary.** Installing the app from Products after running tests against stashed HEAD silently installs the wrong build — the fixed behavior "stops working" for no visible reason (hit 2026-07-23: `-OPEN_SECONDARY routeCreator` went dead because the installed parse() predated the hook). Rebuild from the working tree before any simulator smoke that follows a stash/checkout validation cycle.
...
### 2026-07-21 — The Apple sign-in chain is correctly configured; `sign_in_failed` throws away the only diagnostic that matters

Trigger: WP-52 inspected all five links of the Apple sign-in chain against live 1.1.1 (25) after 12 `ASAuthorizationError` code-1000 events across 4 devices and 0 first-time completions. Every link came back correct, so six weeks of app-side debugging had been aimed at a configuration hypothesis that the evidence does not support.

- **All five links are confirmed correct, and this is now settled — do not re-investigate them.** Both Apple-issued profiles for `com.runsmart.lite` carry `com.apple.developer.applesignin: [Default]`, including the App Store **distribution** profile issued 2026-07-19 (verified via `codesign -d --entitlements :-` on the archive plus `security cms -D -i` on the profile). Supabase is proven working by a matched pair on 2026-07-21: `user_signedup` / `provider: apple` / `grant_type: id_token` / **status 200** at 07:27:34Z, and a PostHog `sign_in_completed` from iOS 1.1.1 build 25 (`$is_testflight=false`) at 07:27:35.9Z. The Resumely June root cause (Services ID / .p8 never completed) is **not** this bug: those artifacts are only used by the web authorization-code flow and cannot raise an on-device `ASAuthorizationError`.
- **`Analytics.trackSignInFailed` records only `nsError.domain` and `nsError.code`, which makes every code-1000 unfalsifiable.** `ASAuthorizationError` 1000 is `.unknown` — a wrapper. The real cause is in `userInfo[NSUnderlyingErrorKey]` (an AuthKit/ AKAuthenticationError domain + code) and `localizedFailureReason`, and the app discards both. No further hypothesis about this bug can be tested until those are captured.
- **To make an already-authorized device behave like a first-time user, revoke the app, don't buy a device.** Settings → Apple ID → Sign-In & Security → Sign in with Apple → RunSmart → Stop Using Apple ID resets to the full first-time consent sheet. Deleting the Supabase user does **not** do this — it leaves the Apple-side authorization intact, which is why the 2026-07-20 and 2026-07-21 "clean install" tests both passed on the founder's iPhone14,5 and proved nothing about first-time consent. That trap has now produced two misleading green results.
- **Check whether the telemetry existed before treating a date range as a regression window.** `sign_in_failed` first shipped 2026-07-14 (`5fa2e00`), so pre-build-23 failure counts are unknown, not zero — an initial read of "failures start 2026-07-15" was an instrumentation artifact. The regression signal that *does* survive is the one measured by an event that existed throughout: non-founder `sign_in_completed` went 7-from-40-launchers in June to 0-from-~26 in July (p≈0.007), so the change is real and is not explained by traffic volume.

Future rule: When an on-device SDK error is a generic `.unknown`/code-1000 wrapper, capture the underlying error before forming any theory about it, and validate a "first-time user" repro by revoking the app's authorization rather than by clearing app or server state.

### 2026-07-21 — Supabase `auth.identities` settles "regression vs. always broken" when event telemetry cannot

Trigger: WP-52a step 3 challenged EXD-023's regression framing with a competing reading — if June's 7 successes were *also* already-authorized Apple IDs, nothing regressed and first-time sign-in may have been broken all along. July's data cannot distinguish the two, and the app emits no "was this a first authorization" property.

- **The provider identity table answers it directly.** `SELECT date_trunc('month', i.created_at), count(*) FROM auth.identities i JOIN auth.users u ON u.id = i.user_id WHERE i.provider = 'apple' GROUP BY 1` returned **7 Apple identities in June (2026-06-03 → 2026-06-18), all brand-new users**, and **1 in July** — the founder's own 07-21 test. An identity row is only created on a first authorization for that provider, so June's successes were genuine first-time authorizations. The regression framing holds: first-time Apple sign-in worked 7 times in June and has not worked once since **2026-06-18**.
- **Unlike Supabase auth *logs*, the auth *tables* are not subject to 24h retention.** WP-52 had to record "the 2026-07-15 failures are unrecoverable" because `get_logs` only reaches back a day. `auth.users` / `auth.identities` persist for the project's lifetime, so historical signup shape stays queryable long after the logs roll off. Reach for the tables, not the logs, for anything older than a day.
- **One caveat that will otherwise mislead:** deleting a Supabase user deletes its identity row, so a later re-signin creates a *new* identity even though Apple-side consent was never revoked. That is exactly what the founder's 07-21 row is. An identity row proves first-time-in-Supabase; it only proves first-time-in-Apple when the account was never deleted.

Future rule: Before accepting or rejecting a "regression" framing, look for a durable server-side table that records the event's first occurrence per user. Prefer it over analytics events, which may postdate the behavior, and over auth logs, which expire.

**Superseded 2026-07-22 on the conclusion, not the method.** The revoke-and-retry test showed first-time Apple sign-in *does* work on 1.1.1 (25), so the "has not worked since 2026-06-18" reading was wrong. The identity-table method is still right and still worth reusing; what it could not see is that **Apple's `sub` is stable across revoke and re-authorize**, so a new identity row proves "new to Supabase", never "new to Apple". See the entry below.

### 2026-07-22 — "Zero successes since date X" is not evidence that a path is broken for everyone

Trigger: WP-52 and WP-52a concluded that first-time Apple sign-in had been broken since 2026-06-18, from 7 stranger signups in June, 0 after, and 10 code-1000 failures across 4 device identifiers. A revoke-and-retry test on public 1.1.1 (25) then completed a genuine first-time sign-in on the first session, refuting it.

- **A zero-success window at low traffic cannot distinguish "broken for everyone" from "broken for some".** The July cohort was ~26 launchers with a handful of sign-in attempts; a per-user failure cause affecting a minority produces the same zero as a total outage. The p≈0.007 figure computed against June's rate assumed the failure was uniform, which is what was in question — the statistic could not test its own premise.
- **Check whether a failing device identifier is a real product.** `iPhone99,7` appeared among the four failing devices and is not an Apple model, so it is a simulator, spoofed, or automated client despite `$is_emulator=false`. One of every four "affected devices" was not a user. Validate model identifiers against real hardware before counting devices as evidence.
- **Sign in with Apple has device- and account-side preconditions the app cannot see:** the device must be signed into iCloud and the Apple ID must have two-factor authentication enabled. Both failures surface as a bare `ASAuthorizationError` 1000, indistinguishable from an app fault without the underlying error. Treat "code 1000 on some devices only" as an environment hypothesis first, not a code or configuration hypothesis.
