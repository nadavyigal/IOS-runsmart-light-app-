...
## Active Rules
- **`ASAuthorizationError` code 1000 with `has_underlying_error: false` means the device has no usable Apple Account — it is not a bug in our Apple leg, and no code change fixes it.** Reproduced on a clean simulator 2026-07-26: with no Apple Account signed in, tapping Sign in with Apple makes iOS show its own "Sign in to your Apple Account" alert, and dismissing it returns a bare code 1000. The emitted `sign_in_failed` was byte-identical to every failing production session. The same shape comes from a managed/restricted Apple ID or MDM restrictions. **Rule: never treat a bare 1000 as something to fix in the request** (nonce, scopes, controller, entitlement — all were already correct). The only remedy is a sign-in path that does not depend on Apple, which is why email now exists. Corollary for diagnosis: spending weeks instrumenting a bare 1000 cannot converge, because the information you want was never in the error.
