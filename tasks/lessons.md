...
## Active Rules
- **App Store Review traffic looks exactly like real users and will fake an activation signal if you let it.** PostHog 171597 carries a Cupertino cluster — one short session per submitted build, ascending build numbers, rotating device models, `en` / `America/Los_Angeles`, `$is_testflight = False`, and never a single completed sign-in. On 2026-07-26 all three "real non-founder devices failing sign-in" that a work packet had built a decision on turned out to be reviewer sessions. **Two cheap discriminators, use both before calling any iOS cohort organic:** (1) compare the event's `app_build` against that build's *public release timestamp* — a reviewer runs the binary hours before it goes live, and a pre-release build on a non-TestFlight device is definitionally not a user; (2) check `$geoip_city_name = 'Cupertino'`. `$is_testflight` alone is not enough — reviewers install the real build, so it reads False exactly like a store install. Note this is the mirror image of the 2026-07-19 global lesson: over-aggressive exclusion *deletes* signal, and no exclusion *manufactures* it. Partition traffic into review / TestFlight / other and confirm the buckets reconcile to the daily person totals.
- **To prove a shipped iOS build emits telemetry, inspect the archive and look for review-session events — do not wait for a user.** The uploaded `.xcarchive` under `~/Library/Developer/Xcode/Archives/<date>/` is the binary Apple distributes; `PlistBuddy -c "Print :POSTHOG_API_KEY" "$APP/Info.plist"` compared byte-for-byte against the project's own token settles the empty-key failure mode in one command, without printing the secret. Better still, App Store Review runs that exact build a few hours before release, so its session is a free production smoke test: if `app_version` / `app_build` appear in the project at all, the pipe works. Post-release silence at a rate of 0–4 persons/day is then low traffic, not breakage — check for existing zero-activity days before calling an outage.
