...
- [x] **Packet step 2 — DONE 2026-07-20.** Built `origin/main` (`9bc2e29`) to iPhone 13 / iOS 26.5.2 and launched 11:36:39 UTC. PostHog 171597 received `app_version=1.1.1` / `app_build=25` on 12 of 13 events at 11:36:47-53 UTC (`os_version=26.5.2`, `iPhone14,5`, distinguishing it from the 09:58-10:03 simulator runs on OS 26.5). All three event sources covered: autocapture, the `track()` wrapper, and the direct `PostHogSDK.capture` path.
- [ ] **Follow-up (new, non-blocking) — install/update events cannot carry build identity.** `Application Installed` (7/7 untagged) and `Application Updated` (5/5 untagged) fire inside `PostHogSDK.shared.setup(config)`, which returns before the `register()` call on the next lines. Not a WP-51 regression, but it means **install counts cannot be split by build**, which matters to the activation-cliff analysis. Options if this becomes load-bearing: capture install/update manually after `register()`, or accept the gap and split installs by first-seen `app_launched` instead. Do not attempt to call `register()` before `setup()` — it is a documented no-op.
...
- [x] Wrap PreRunView content in a `ScrollView` so Last Run is reachable on short screens.
- [x] Keep the tall-screen layout stable with a `minHeight` frame.
- [x] Relabel the decorative route preview honestly; remove the `GPS preview` claim.
- [x] Run clean Debug simulator build.
- [x] Device-QA PreRun preview and Last Run reachability on iPhone 17 simulator.
- [x] Device-QA PreRun preview and Last Run reachability on iPhone SE simulator.
- [x] Save visual evidence screenshots under `docs/qa/reports/assets-2026-07-08-wp37-s8/`.
...
- `xcodebuild ... -destination 'platform=iOS Simulator,name=iPhone 17' ... -quiet build` passed; known HealthKit `HKWorkout` initializer deprecation warning and run-script dependency note only.
- iPhone 17 simulator (`-RUNSMART_DEMO_MODE -INITIAL_TAB Run`): preview label shows `Route sketch` instead of `GPS preview`, and `Last Run` is visible/reachable on the Run tab.
- iPhone SE simulator (`-RUNSMART_DEMO_MODE -INITIAL_TAB Run`): preview label shows `Route sketch`; after one scroll, `Last Run` is reachable and visible.
- No unit test added because S8 is a SwiftUI layout/copy honesty fix with acceptance validated visually on both required widths.
- Evidence screenshots saved under `docs/qa/reports/assets-2026-07-08-wp37-s8/`.
...
### Checklist
- [x] Confirm PR #71 was mislabeled as WP-26 and is canonically WP-27 founder-run evidence work.
...
- [x] Run full XCTest suite and confirm the two WP-27 tests execute.
- [x] Verify official Garmin Connect tile provenance or record founder blocker.
- [x] Commit and push to existing PR #72.
...
**Objective:** WP-27 — Garmin Gate-4 evidence recapture runbook.
**Status:** Complete locally after numbering correction. Evidence runbook implemented; actual screenshots, asset provenance confirmation, App Store/TestFlight verification, and Garmin reply remain founder-run steps.
**Branch:** historical PR #71 branch was named `codex/wp26-garmin-gate4-evidence`, but the work is canonically WP-27.

### Checklist
- [x] Rename mistakenly framed WP-26 evidence runbook to WP-27.
...
- [x] Session: confirmed official Garmin Connect tile provenance and replaced prior JPEG derivative with Garmin-served iOS PDF.
- [ ] Founder: verify package against Garmin brand PDF and send/clarify ticket reply.

### Validation - 2026-07-02
- Official Garmin tile provenance verified from `https://developer.garmin.com/brand-guidelines/connect/`; prior local JPEG derivative was replaced with Garmin-served `gc-app-tile_iOS.pdf`.
...
**Objective:** Superseded local Garmin track initiation note.
**Status:** Superseded. Canonical WP-25 lives in Agentic OS executive-os/work-packets, not this repo.
...
- [x] Replace `docs/specs/wp25-garmin-track.md` with a pointer to canonical Agentic OS work-packet specs.
- [x] Keep WP-24 paused and out of scope.
- [x] Run planning-only static verification.
- [x] Commit planning artifact.

### Notes
- This repo does not own WP-25 or WP-26. Current Gate-4 next move is WP-27 founder/live-device `1.0.7 (20)` recapture, brand-PDF verification, then Garmin reply package.
